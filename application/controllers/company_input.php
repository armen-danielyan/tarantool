...
class Company_Input extends CI_Controller {
    ...
    {
        $pagename = "proEO Company Input";
        $oneliner = "One-liner here for Company Input";
        ...
        else 
        {
			...
			$value_elements_set = $this->jedoxapi->array_element_filter($value_elements, "VE_100");
			...
			if($Update == "Update")
			{
				//echo "do update codes!";
				foreach($value_elements_set_alias as $row)
				{
					$var_data = $this->input->post('var_'.$row['element']);
					//save only those with values
					if($var_data != '')
					{
						$update_path = $project.",".$year.",".$row['element'].",".$customer.",".$benefit_element_be_000.",".$cost_of_goods_elements_cg_000;
            ...
            $tc_area = $project.",".$year.",".$value_elements_set_area.",".$customer.",".$benefit_element_be_000.",".$cost_of_goods_elements_cg_000;
            ...
            $this->load->view("company_input_view", $alldata);
        }
    }
