...
class Input_Price_Simulation extends CI_Controller {
    ...
    public function index ()
    {
        ...
        else 
        {
            ...
            $cube_names = "Primary,Raw_Material_Rate,Bom,Secondary,Production,Product_Report,#_Version,#_Year,#_Month,#_Primary_Value,#_Account_Element,#_Receiver,#_Raw_Material_Rate_Value,#_Raw_Material,#_Bom_Value,#_Product,#_Sender,#_Report_Value";
			...
			// PRIMARY COSTS //
			//$pc_area = $version.",".$year.",".$month.",".$primary_value_TQ_area.",".$account_element_CE_Primary_area.",*"; // old
			//$primary_cells = $this->jedoxapi->cell_export( $server_database['database'], $primary_cube_info['cube'], 10000, "", $pc_area ); // old
			
			$pc_area = $version.",".$year.",".$month.",".$primary_value_TQ_area.",".$account_element_ce_primary_all_area.",".$receiver_AR_area;
			...
			// extra post data based on contents of tab 1 and 2.. exclude quantity 0 and below
			$update_rawmat_paths = '';
			...
			$table_data1 = $table_data2 = $table_data3 = $table_data4 = $table_data5 = $table_data6 = $table_data7 = $table_data8 = '';
			...
			switch( $step )
			{
				...
				$table_data1 = $this->jedoxapi->cell_export($server_database['database'],$primary_cube_info['cube'],10000,"",$version.":".$version_sim.",".$path, "", 1, "", "0");

				// Cube Primary: Copy UPC
				$path = $year.",".$month.",".$primary_value_UPC_area.",".$account_element_CE_Primary_area.",".$receiver_RP_area;
				$result = $this->jedoxapi->cell_copy( $server_database['database'], $primary_cube_info['cube'], $version.",".$path, $version_sim.",".$path, 1 );
				$table_data3 = $this->jedoxapi->cell_export($server_database['database'],$primary_cube_info['cube'],10000,"",$version.":".$version_sim.",".$path, "", 1, "", "0");
				...
				$table_data2 = $this->jedoxapi->cell_export($server_database['database'],$raw_material_rate_cube_info['cube'],10000,"",$version.":".$version_sim.",".$path, "", 1, "", "0");
				...
				$table_data4 = $this->jedoxapi->cell_export($server_database['database'],$secondary_cube_info['cube'],10000,"",$version.":".$version_sim.",".$path, "", 1, "", "0");
				
				// Cube Secondary: Copy QTY04
				$path = $year.",".$month.",".$secondary_value_QTY04_area.",".$sender_as_area.",".$receiver_AR_area;
				$result = $this->jedoxapi->cell_copy( $server_database['database'], $secondary_cube_info['cube'], $version.",".$path, $version_sim.",".$path, 1 );
				$table_data7 = $this->jedoxapi->cell_export($server_database['database'],$secondary_cube_info['cube'],10000,"",$version.":".$version_sim.",".$path, "", 1, "", "0");
				...
				$table_data5 = $this->jedoxapi->cell_export($server_database['database'],$bom_cube_info['cube'],10000,"",$version.":".$version_sim.",".$path, "", 1, "", "0");
				...
				$table_data8 = $this->jedoxapi->cell_export($server_database['database'],$production_cube_info['cube'],10000,"",$version.":".$version_sim.",".$path, "", 1, "", "0");

				// Cube Production: Copy QTY07
				$path = $year.",".$month.",".$production_value_QTY07_area.",".$product_AP_area;
				$result = $this->jedoxapi->cell_copy( $server_database['database'], $production_cube_info['cube'], $version.",".$path, $version_sim.",".$path, 1 );
				$table_data6 = $this->jedoxapi->cell_export($server_database['database'],$production_cube_info['cube'],10000,"",$version.":".$version_sim.",".$path, "", 1, "", "0");
				
				//update data based on 2 tabs. calculation of paths has to be done here for version_sim to be used.					
				foreach($raw_mat_data as $row)
				{
					if( $row['quantity'] == 0 ) { continue; }
					$update_rawmat_paths .= $version_sim.",".$year.",".$month.",".$raw_material_value_P002_area.",".$row['element'].":";
				}
				...
				foreach($primary_data as $row)
				{
					...
					if( $row['quantity'] == 0 ) { continue; }
					$update_primary_paths .= $version_sim.",".$year.",".$month.",".$primary_value_UPC_area.",".$row['element'].",".$receiver_AR_area.":";
				}
				$update_primary_paths = rtrim($update_primary_paths, ":");

				// Splash option 3 sets the average price as the base cell
				$update_primary = $this->jedoxapi->cell_replacebulk($server_database['database'], $primary_cube_info['cube'], $update_primary_paths, $update_primary_val, '', 3);
				...
				if($this->check_file($folder_base_path) == 1)
				{
					...
					// execute etl
					$version_sim_name1 = $this->jedoxapi->get_name($version_elements, $version_sim);
					/*
				...
				case 3: //merge
					...
					//display
					$disp1 = $version.":".$version_sim.",".$year.",".$month.",".$primary_value_TQ_area.",".$account_element_CE_Primary_area.",".$receiver_RP_area;
            ...
            $alldata = array(
                ...
                "version"      => $version,
                "version_sim_name1" => $version_sim_name1,
                "step"         => $step,
                "error"        => $error,
                //"return"       => $return,
                //"id"           => $id,
                ...
                "table_data6"  => $table_data6,
                "table_data7"  => $table_data7,
                "table_data8"  => $table_data8,
