            ...
            <div class="content_div">
                ...
                <table id="tb0" class="avtable_2">
                	...
                	<tr>
                		...
                		<td class="label"><a href="#" onclick="return false;" id="showform">Run New Calc</a></td>
                		<td>&nbsp;</td>
                	</tr>
                </table>
                ...
                <table id="tb1" class="avtable_2">
					...
					$equipment_base = $this->jedoxapi->dimension_elements_base( $equipment_alias );
					
					// Filter Equipment and only keep parents
					$equipment_parents = $equipment_alias;
					foreach( $equipment_parents as $key => $brow )
					{
						if( $brow['number_children'] == 0 )
						{
							unset( $equipment_parents[$key] );
						}
					}
					...
					foreach( $table1_data as $row )
					{
						if( $row['value'] != 0 )
						{
							$paths = explode(",", $row['path']); // Run_Number, Version, Equipment, KPI, KPI, Value
							
							if( $paths[3] == $paths[4] ) { continue; }
							
							$equipment = '';
							foreach( $equipment_base as $brow )
							{
								if( $brow['element'] == $paths[2] )
								{
									$equipment = $brow['name_element'];
									break;
								}
							}
							...
							$equipment_parent = "";
							foreach( $equipment_parents as $bparent )
							{
								if( $bparent['element'] == $brow['parents'] )
								{
									// Find the name of the grand-parent
									foreach( $equipment_parents as $bgparent )
									{
										if( $bgparent['element'] == $bparent['parents'] )
										{
											// Find the name of the grand-grand-parent
											foreach( $equipment_parents as $bggparent )
											{
												if( $bggparent['element'] == $bgparent['parents'] )
												{
													$equipment_parent = $bggparent['name_element'];
													break 3;
												}
											}
							...
							$kpi_1 = '';
							foreach( $mfg_kpi_1_base as $row1 )
							{
								if( $row1['element'] == $paths[3] )
								{
									$kpi_1 = $row1['name_element'];
									break;
								}
							}							
							if( $kpi_1 == '' ) { continue; }
							...
							$kpi_2 = '';
							foreach( $mfg_kpi_2_base as $row2 )
							{
								if( $row2['element'] == $paths[4] )
								{
									$kpi_2 = $row2['name_element'];
									break;
								}
							}							
							if( $kpi_2 == '' ) { continue; }
							...
							echo '<tr>';
							echo '<td class="label">' . $equipment_parent . '</td>'; 
							echo '<td class="label">' . $equipment . '</td>';
							echo '<td class="label">' . $kpi_1 . '</td>';
							echo '<td class="label">' . $kpi_2 . '</td>';
							echo '<td>' . number_format( $row['value'] * 100, "4", ".", ",") .' %</td>';
							echo '</tr>'."\r\n";
						}
					}
...
<div id="dialog-form" title="Run New Calc">
  <form id="run_etl">
	  ...
	  <?php
			...
			$dbc = 0;
      		foreach($date_base as $row)
      		{
      			$dbc += 1;
      	?>
			<option value="<?php echo $row['element']; ?>" <?php if($date_now == $row['name_element']){ ?>selected="selected"<?php } ?> ><?php echo $row['name_element']; ?></option>
		<?php		
      		}
      	?>
      </select>
