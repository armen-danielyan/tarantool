    ...
    <tr>
        <td id="tsidebar">
            ...
            <div id="filter_menu">
                <h3>Filters</h3>
                <div>
                    <form id="form1" name="form1" method="post" action="<?php echo site_url('efficiency_products_details_v2'); ?>">
        ...
        <td class="tcontent" rowspan="2">
            <?php
			$breadcrumb = anchor('home', 'Home', array('title' => 'Go to Home'))." > ".anchor('efficiency_products_v2', 'Products', array('title' => 'View Efficiency Products'))." > <span class='orange'>Details</span>";
                ...
                <table id="tb1" class="avtable_2 controller">
					...
					<?php
						...
						foreach($product_set_alias as $row)
						{
							...
							{
								$paths = explode(",", $drow1['path']);
								if($version_a == $paths[0] && $row['element'] == $paths['6'])
								{
									$a = $drow1['value'];
								}
								if($version_t == $paths[0] && $row['element'] == $paths['6'])
								{
									$b = $drow1['value'];
								}
								if($version_p == $paths[0] && $row['element'] == $paths['6'])
								{
									$c = $drow1['value'];
								}
							}
							...
							foreach($table5d_data as $drow1)
							{
								...
								if($version_AP == $paths[0] && $row['element'] == $paths['6'])
								{
									$l = $drow1['value'];
								}
							}
					...
					<?php
						...
						} // closing tag for the condition in the main recovery row.
					?>
