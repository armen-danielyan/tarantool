...
<table class="content_block" cellpadding="0" cellspacing="0">
	<tr>
		...
		<td class="tcontent" rowspan="2">
			...
			<ul id="sortable_charts">
				<?php 
					$query = $this->proeo_model->get_charts($jedox_user_details['name'], $this->session->userdata('jedox_db'));
					if($query->num_rows() > 0)
					{
						foreach($query->result_array() as $row_data)
						{
				?>
				<li id="sc_<?php echo $row_data['id']; ?>">
                    <div class="handle" style="background-color: #eeeeee; height: 16px;">
                        <?php echo $row_data['chart_name']; ?><span class="ui-icon ui-icon-close right deletechart" style="margin-right: .3em;" ></span>
                    </div>
                    <iframe src="<?php echo site_url($row_data['chart_url']); ?>" style="width: 460px; height: 280px; border:none;" scrolling="no" ></iframe>
                    <a href="<?php echo site_url($row_data['chart_link']) ?>" title="View More Details"><span class="ui-icon ui-icon-search right" style="margin-right: .3em;" ></span></a>
			    </li>
				<?php
						}
					}
				...
				?>
				<li>
					<h3>Hi <?php echo $jedox_user_details['name']; ?>!</h3>
					<p>You currently do not have any pinned charts. You can pin charts here and customize you home by pressing the "PIN" icon located above specific charts.</p>
				</li>
				<?php
					}
				?>
