...
<title>proEO | Efficiency Processes</title>
    ...
    <tr>
        <td id="tsidebar">
            ...
            <div id="filter_menu">
                <h3>Filters</h3>
                <div>
                    <form id="form1" name="form1" method="post" action="<?php echo site_url('efficiency_processes_v2'); ?>">
        ...
        <td class="tcontent" rowspan="2">
            <?php
			$breadcrumb = anchor('home', 'Home', array('title' => 'Go to Home'))." > <span class='orange'>Processes</span> > ".anchor('efficiency_processes_details_v2', 'Details', array('title' => 'View Efficiency Processes Details'));
			$this->load->view("header", array("breadcrumb" => $breadcrumb)); 
			?>
            <div class="advance_details">
                <?php 
                if($this->jedox->page_permission($jedox_user_details['group_names'], "efficiency_processes_details"))
                {
                    //echo anchor("efficiency_processes_details_v2", "<span class='ui-icon ui-icon-search right' ></span>", array('title' => 'View Efficiency Processes Details')); 
                }
                ?>
            </div>
            <div class="tabber">
            ...
            <div class="content_div" >
                <table id="tb1" class="avtable_2 controller">
                	<tr data-level="header" class="header">
						...
						<td style="min-width:20px !important; text-align:center !important;">&nbsp;</td>
                    	<td class="center" colspan="4">Total Cost/Business Process</td>
						<td style="min-width:20px !important; text-align:center !important;">&nbsp;</td>
						<td style="min-width:20px !important; text-align:center !important;">&nbsp;</td>
						<td class="center" colspan="4">Output</td>
					</tr>
                	<tr data-level="header" class="header">
						<td>&nbsp;</td>
						<td style="min-width:20px !important; text-align:center !important;">&nbsp;</td>
					...
					<?php
						$depth_stack = array();
						foreach ($process_set_alias as $row)
						{
							//arrow indicators
							$arrow1 = '';
							$arrow2 = '';
							...
							foreach($table2_data as $drow2)
							{
								$paths = explode(",", $drow2['path']);
								if($paths[0] == $version_a && $paths[4] == $row['element'])
								{
									$e = $drow2['value'];
								}
							...
							foreach($cells_process_attributes as $drow3)
							{
								$paths = explode(",", $drow3['path']);
								if($row['element'] == $paths[1])
								{
									$uom = $drow3['value'];
								}
							}
							if($row['name_element'] == "Business Processes")
							{
								$row['name_element'] = "Total";
								$rowclass = "class='tmain'";
							}
							...
							{
								//if($row['name_element'] == 'Processes') // seems to be total now in alias
								if($row['name_element'] == 'Total') // seems to be total now in alias
								{
									//$row['name_element'] = 'Total';
									$rowclass = "class='tmain header'";
									//echo "hiadadas";
									$depth = '';
					?>				
					<tr <?php echo $rowclass; ?> data-level="<?php echo $true_depth; ?>" >
					<?php				
								}
								else 
								{
					?>				
					<tr <?php echo $rowclass; ?> data-level="<?php echo $true_depth; ?>" id="level_<?php echo $true_depth."_".$sub_depth; ?>">
					<?php				
								}
					?>
						...
						<td><?php echo number_format(($g* -1), 0, ".", ","); ?></td>
						<td><?php echo $uom; ?></td>
					</tr>
					<?php	
							}
